...
| `content-scorecard` | `content-scorecard.workflow.json` | Cron 10h + Manual | L0 | Audit refs | — | Sheet metrics → score; upsert scorecard (P4.4 skip khi writeback off) | staging |
...
| `content-page-publish` | `content-page-publish.workflow.json` | Cron 10/14/18h + Manual | L0 | Consume | — | **Page Publish P4.3/4:** Postgres due → Graph → mark Postgres; metrics Sheet optional (`CONTENT_SHEET_WRITEBACK_ENABLED`) | staging |
