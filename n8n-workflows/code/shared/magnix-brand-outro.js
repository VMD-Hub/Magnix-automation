...
const MAGNIX_MOTIVATE_VARIANTS = [
  {
    id: 1,
    short: 'Mọi hành trình vạn dặm đều bắt đầu từ một bước chân. Đừng ngại bắt đầu ngay hôm nay.',
  ...
  {
    id: 3,
    short: 'Chưa ai về đích nếu chưa dám bước đi. Hãy bắt đầu hành trình về nhà của bạn ngay bây giờ.',
...
function pickBrandVariantIndex(seed) {
  const n = Number(seed);
...
function getMotivateVariant(variantId) {
  return (
    MAGNIX_MOTIVATE_VARIANTS.find((v) => v.id === variantId) || MAGNIX_MOTIVATE_VARIANTS[0]
  );
}

function buildBrandOutroBeat(opts = {}) {
...
function hasBrandOutroBeat(beats) {
  return (beats || []).some((b) => String(b?.role || '').toLowerCase() === 'brand_outro');
}

function appendBrandOutroToBeats(beats, opts = {}) {
...
function estimateDurationWithBrandOutro(durationSec, opts = {}) {
...
if (typeof globalThis !== 'undefined') {
  globalThis.MAGNIX_BRAND_TAGLINE = MAGNIX_BRAND_TAGLINE;
  globalThis.MAGNIX_BELL_SFX_URL = MAGNIX_BELL_SFX_URL;
  globalThis.MAGNIX_MOTIVATE_VARIANTS = MAGNIX_MOTIVATE_VARIANTS;
  globalThis.pickBrandVariantIndex = pickBrandVariantIndex;
  globalThis.buildBrandOutroBeat = buildBrandOutroBeat;
  globalThis.appendBrandOutroToBeats = appendBrandOutroToBeats;
  globalThis.hasBrandOutroBeat = hasBrandOutroBeat;
  globalThis.estimateDurationWithBrandOutro = estimateDurationWithBrandOutro;
}
