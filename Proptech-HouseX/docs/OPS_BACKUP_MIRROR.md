...
4. Tạo một token ngẫu nhiên tối thiểu 32 byte **ở máy local** bằng password manager
   hoặc bộ sinh mật mã hệ điều hành. Không đưa token vào đối số command, command
   line, shell history, chat hoặc log. Dùng editor để đặt **cùng một giá trị** vào:
   - `/root/n8n.env`: `HOUSEX_BACKUP_ALERT_TOKEN=...`
   - `/etc/housex/backup.env`: `HOUSEX_BACKUP_ALERT_TOKEN=...`

   `/root/n8n.env` và `/etc/housex/backup.env` phải là `root:root`, mode `0600`.
   Token này chỉ dành cho event có `agent` chính xác `House X Backup`; không tái
   sử dụng `MAGNIX_WEBHOOK_TOKEN`. Trong `/etc/housex/backup.env`, đồng thời đặt
   `HOUSEX_BACKUP_RCLONE_REMOTE=housex-crypt:postgres`, URL webhook HTTPS và giữ
   `HOUSEX_BACKUP_REQUIRE_OFFSITE=true`. Dùng `sudoedit`/editor root-only; không
   dùng `VAR=token command`, `echo token`, `sed ...token...` hoặc paste token vào
   câu lệnh.
...
- Rotate OAuth/rclone token và `HOUSEX_BACKUP_ALERT_TOKEN` ít nhất mỗi 90 ngày hoặc ngay
