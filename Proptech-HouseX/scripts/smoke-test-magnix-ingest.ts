...
async function main() {
  ...
  if (!site) {
    console.error(
      "Thiếu SITE — ví dụ: SITE=http://127.0.0.1:3000 npm run go-live:smoke-magnix-ingest",
    );
    process.exit(1);
  }
  ...
  if (!contentType.includes("application/json")) {
    console.error(
      "FAIL — response không phải JSON (có thể route chưa build hoặc URL sai)",
    );
