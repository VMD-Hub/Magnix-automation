...
function skeletonDef(row: HcmRow): NoxhLandingDef {
  ...
  return {
    ...
    lat: row.lat,
    lng: row.lng,
    description: `${row.name} tại ${row.address}. Chủ đầu tư: ${row.developerName}.${legacy}${tip} Giá và suất đang được xác minh. ${NOXH_UPDATING_SOON}`,
...
Nguồn: phucdat.vn · CafeF · Sở Xây dựng TP.HCM. House X không thu đặt cọc thay CĐT.`,
    highlights: [
      ...
      },
      {
        ...
        text: "Theo House X thuộc TP.HCM; địa chỉ kép giữ Tân Uyên (Bình Dương cũ) để tra cứu.",
      },
    ],
    unitTypes: [
      {
