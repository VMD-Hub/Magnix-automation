...
import {
  allNoxhDakLakDefs,
  buildNoxhDakLakSeedLanding,
} from "@/lib/preview/noxh-dak-lak-projects";

/** Seed 5 NOXH Đắk Lắk — salesRegion CENTRAL. */
export async function seedNoxhDakLakProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhDakLakDefs()) {
    ...
    const landing = buildNoxhDakLakSeedLanding(def.slug);
    if (!landing) continue;
