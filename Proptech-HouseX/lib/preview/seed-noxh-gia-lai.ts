...
import {
  allNoxhGiaLaiDefs,
  buildNoxhGiaLaiSeedLanding,
} from "@/lib/preview/noxh-gia-lai-projects";

/** Seed 7 NOXH Gia Lai — salesRegion CENTRAL. */
export async function seedNoxhGiaLaiProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhGiaLaiDefs()) {
    ...
    const landing = buildNoxhGiaLaiSeedLanding(def.slug);
    if (!landing) continue;
