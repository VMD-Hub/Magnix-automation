...
import {
  allNoxhKhanhHoaDefs,
  buildNoxhKhanhHoaSeedLanding,
} from "@/lib/preview/noxh-khanh-hoa-projects";

/** Seed 7 NOXH Khánh Hòa — salesRegion CENTRAL. */
export async function seedNoxhKhanhHoaProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhKhanhHoaDefs()) {
    ...
    const landing = buildNoxhKhanhHoaSeedLanding(def.slug);
    if (!landing) continue;
