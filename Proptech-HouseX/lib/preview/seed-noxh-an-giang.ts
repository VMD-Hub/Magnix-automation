...
import {
  allNoxhAnGiangDefs,
  buildNoxhAnGiangSeedLanding,
} from "@/lib/preview/noxh-an-giang-projects";

/** Seed 8 NOXH An Giang — salesRegion SOUTH. */
export async function seedNoxhAnGiangProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhAnGiangDefs()) {
    ...
    const landing = buildNoxhAnGiangSeedLanding(def.slug);
    if (!landing) continue;
