...
import {
  ...
  buildNoxhQuangNinhSeedLanding,
} from "@/lib/preview/noxh-quang-ninh-projects";

/** Seed 7 NOXH Quảng Ninh — salesRegion NORTH. */
export async function seedNoxhQuangNinhProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhQuangNinhDefs()) {
    ...
    const landing = buildNoxhQuangNinhSeedLanding(def.slug);
    if (!landing) continue;
