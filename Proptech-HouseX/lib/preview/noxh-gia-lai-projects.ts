...
function skeletonDef(row: GlRow): NoxhLandingDef {
  ...
  return {
    ...
    lat: row.lat,
    lng: row.lng,
    description: `${row.name} tại ${row.address}. CĐT theo danh mục House X: ${row.developerName}. Thuộc tỉnh Gia Lai sau sắp xếp (Bình Định cũ nếu Quy Nhơn / An Nhơn).${tip} Skeleton — giá / suất cập nhật khi có Sở XD hoặc CĐT. Tư vấn điều kiện qua House X.`,
    ...
    heroSubtitle: `${seoName} · ${row.district}, Gia Lai${tip} — catalog House X`,
    locationNotes: `${row.name}: ${row.address}.

Sau NQ 2025, Bình Định sáp nhập vào tỉnh Gia Lai mới. Tọa độ ước lượng.

Đối chiếu soxaydung.gialai.gov.vn. House X: wiki NOXH + form tư vấn.`,
    highlights: [
    ...
    faqs: [
      {
        ...
        a: "Canonical House X: Gia Lai (sau NQ 2025). Tên Bình Định / Quy Nhơn / Pleiku / An Nhơn vẫn dùng để tìm kiếm.",
      },
      {
        q: "Giá bao nhiêu?",
