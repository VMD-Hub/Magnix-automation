...
import {
  allNoxhDongThapDefs,
  buildNoxhDongThapSeedLanding,
} from "@/lib/preview/noxh-dong-thap-projects";

/** Seed 6 NOXH Đồng Tháp — salesRegion SOUTH. */
export async function seedNoxhDongThapProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhDongThapDefs()) {
    ...
    const landing = buildNoxhDongThapSeedLanding(def.slug);
    if (!landing) continue;
