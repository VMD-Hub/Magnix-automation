...
import {
  allNoxhLamDongDefs,
  buildNoxhLamDongSeedLanding,
} from "@/lib/preview/noxh-lam-dong-projects";

/** Seed 6 NOXH Lâm Đồng — salesRegion SOUTH. */
export async function seedNoxhLamDongProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhLamDongDefs()) {
    ...
    const landing = buildNoxhLamDongSeedLanding(def.slug);
    if (!landing) continue;
