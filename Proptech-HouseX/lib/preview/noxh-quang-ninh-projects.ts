...
function skeletonDef(row: QnRow): NoxhLandingDef {
  ...
  return {
    ...
    lat: row.lat,
    lng: row.lng,
    description: `${row.name} tại ${row.address}. Chủ đầu tư: ${row.developerName}. Tỉnh Quảng Ninh — không sáp nhập NQ 2025.${tip} Giá và suất đang được xác minh. ${NOXH_UPDATING_SOON}`,
