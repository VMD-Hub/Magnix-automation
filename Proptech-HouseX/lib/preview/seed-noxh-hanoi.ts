...
import {
  allNoxhHanoiDefs,
  buildNoxhHanoiSeedLanding,
} from "@/lib/preview/noxh-hanoi-projects";

/** Seed 17 skeleton NOXH Hà Nội — salesRegion NORTH. */
export async function seedNoxhHanoiProjects(prisma: PrismaClient): Promise<void> {
  for (const def of allNoxhHanoiDefs()) {
    ...
    const landing = buildNoxhHanoiSeedLanding(def.slug);
    if (!landing) continue;
