...
import {
  allNoxhBacNinhDefs,
  buildNoxhBacNinhSeedLanding,
} from "@/lib/preview/noxh-bac-ninh-projects";

/** Seed 8 NOXH Bắc Ninh — salesRegion NORTH. */
export async function seedNoxhBacNinhProjects(
  prisma: PrismaClient,
): Promise<void> {
  for (const def of allNoxhBacNinhDefs()) {
    ...
    const landing = buildNoxhBacNinhSeedLanding(def.slug);
    if (!landing) continue;
