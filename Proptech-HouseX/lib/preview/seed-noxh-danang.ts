...
import {
  allNoxhDanangDefs,
  buildNoxhDanangSeedLanding,
} from "@/lib/preview/noxh-danang-projects";

/** Seed 5 NOXH Đà Nẵng — salesRegion CENTRAL. */
export async function seedNoxhDanangProjects(prisma: PrismaClient): Promise<void> {
  for (const def of allNoxhDanangDefs()) {
    ...
    const landing = buildNoxhDanangSeedLanding(def.slug);
    if (!landing) continue;
