...
type Props = {
  listingId?: string;
  projectId?: string;
  /** Tiêu đề form — mặc định theo ngữ cảnh. */
  title?: string;
...
function defaultTitle(listingId?: string, projectId?: string) {
...
export function LeadContactForm({
  ...
  const heading = title ?? defaultTitle(listingId, projectId);
  ...
  if (done) {
    return (
      ...
      >
        <p className="font-semibold text-emerald-900">Đã gửi yêu cầu tư vấn</p>
        <p className="mt-2 text-sm text-emerald-800">
          Chuyên viên sẽ liên hệ trong giờ làm việc. Bạn có thể theo dõi trạng thái tại{" "}
  ...
  return (
    ...
    >
      <p className={compact ? "text-xs font-semibold text-slate-800" : "text-lg font-bold text-slate-900"}>
        {heading}
      </p>
      {!compact && (
        <p className="mt-1 text-sm text-slate-600">
          Ghi nhận nhu cầu gắn với {listingId ? "tin đăng" : "dự án"} này — môi giới hoặc
          vận hành HouseX sẽ phản hồi trong giờ làm việc.
        </p>
      )}
      ...
      <label className="mt-3 flex items-start gap-2 text-xs text-slate-600">
        ...
        />
        <span>
          Tôi đồng ý HouseX liên hệ qua điện thoại/email để tư vấn theo thông tin đã cung
          cấp (xem{" "}
      ...
      <div className={`flex gap-2 ${compact ? "mt-3" : "mt-5"}`}>
        <Button type="submit" disabled={loading} className={compact ? "flex-1" : undefined}>
